...
function interesting_things() {
    ...
    16)
        ...
        ;;
    17)
        getnothing https://github.com/possatti/pokemonsay.git pokemonsay
        pkg install clang -y
        pkg in cowsay -y
...
function pentest_tools() {
    ...
    19)
        ...
        ;;
    20)
        python -m pip install termineter
        pentest_tools
        ;;
    21)
        pkg in nmap -y
...
function choosepeo() {
    if cat $mypath/other/people.txt; then
        b=$(cat $mypath/other/people.txt)
    else
        b=0
    fi
    case $b in
...
function detectdepend() {
    if cat $mypath/other/dependent.txt; then
        a=$(cat $mypath/other/dependent.txt)
    else
        a=0
    fi
    if [ $a == 1 ]; then
